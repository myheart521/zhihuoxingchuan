...
import { Search, Refresh, Plus, Edit, Delete, View, Top, Lock } from '@element-plus/icons-vue'
...
const getList = async () => {
  loading.value = true
  try {
    const res = await BlogApi.getBlogPage({
      ...queryParams,
...
const handleDelete = async (row: any) => {
  try {
    ...
    const res = await BlogApi.deleteBlog(row.id)
    if (res) {
...
const handleTop = async (row: any) => {
  try {
    const res = await BlogApi.updateBlogTop(row.id, {
      isTop: row.isTop ? 0 : 1
    })
    console.log("取消置顶或者置顶：",res)
    if (res) {
...
const handleVisibility = async (row: any, visibility: number) => {
  try {
    const res = await BlogApi.updateBlogVisibility(row.id, {
...
<template>
  <div class="app-container">
    ...
    <!-- 操作按钮区域 -->
    <el-card class="mb-4 !border-none" shadow="never">
      <el-button type="primary" :icon="Plus" @click="handleAdd"  v-hasPermi="['blog:blog:create']">新增博客</el-button>
    </el-card>
        ...
        <el-table-column prop="title" label="标题" min-width="200" show-overflow-tooltip>
          <template #default="{ row }">
            <div class="flex items-center">
              <el-tag v-if="row.isTop" type="danger" effect="dark" size="small" class="mr-2"
                >置顶
              </el-tag>
              {{ row.title }}
            </div>
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            <div class="operation-buttons">
              ...
              <el-button link type="primary" :icon="Edit" @click="handleEdit(row)"  v-hasPermi="['blog:blog:update']">编辑</el-button>
              <el-dropdown trigger="click">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item :icon="Top" @click="handleTop(row)"  v-hasPermi="['blog:blog:isTop']">
                      {{ row.isTop ? '取消置顶' : '置顶' }}
                    </el-dropdown-item>
                    <el-dropdown-item :icon="Lock" divided  v-hasPermi="['blog:blog:visibility']">
                    ...
                    <el-dropdown-item :icon="Delete" divided @click="handleDelete(row)"  v-hasPermi="['blog:blog:delete']">
                      删除
                    </el-dropdown-item>
                  </el-dropdown-menu>
                </template>
