package com.example.project.module.ai.service.knowledge;
...
import com.example.project.module.ai.controller.admin.knowledge.vo.segment.AiKnowledgeSegmentPageReqVO;
import com.example.project.module.ai.controller.admin.knowledge.vo.segment.AiKnowledgeSegmentProcessRespVO;
import com.example.project.module.ai.controller.admin.knowledge.vo.segment.AiKnowledgeSegmentSaveReqVO;
import com.example.project.module.ai.controller.admin.knowledge.vo.segment.AiKnowledgeSegmentUpdateStatusReqVO;
...
import com.example.project.module.ai.dal.dataobject.knowledge.AiKnowledgeSegmentDO;
import com.example.project.module.ai.dal.mysql.knowledge.AiKnowledgeSegmentMapper;
...
import static com.example.project.module.ai.enums.ErrorCodeConstants.KNOWLEDGE_SEGMENT_CONTENT_TOO_LONG;
import static com.example.project.module.ai.enums.ErrorCodeConstants.KNOWLEDGE_SEGMENT_NOT_EXISTS;
...
public class AiKnowledgeSegmentServiceImpl implements AiKnowledgeSegmentService {
    ...
    public void deleteKnowledgeSegmentByDocumentId(Long documentId) {
        ...
        try {
            ...
            // 记录日志或进行其他错误处理
            log.error("Failed to delete segment records", e);
        }
        ...
        try {
            ...
            // 记录日志或进行其他错误处理
            log.error("Failed to delete segments from vector store", e);
        }
    ...
    private void writeVectorStore(VectorStore vectorStore, AiKnowledgeSegmentDO segmentDO, Document segment) {
        ...
        Object url = segment.getMetadata().get("url");
        if (url != null && !(url instanceof String && ((String) url).startsWith("http"))) {
            log.error("[writeVectorStore][segmentId: {}] metadata 中 url 字段不合法: {}", segmentDO.getId(), url);
            throw new IllegalArgumentException("segment metadata 中 url 字段不合法: " + url);
        }

        try {
            vectorStore.add(List.of(segment));
        } catch (Exception e) {
            log.error("[writeVectorStore][segmentId: {}] 向量存储 add 失败, segment: {}, metadata: {}", segmentDO.getId(), segment, segment.getMetadata(), e);
            throw e;
        }
